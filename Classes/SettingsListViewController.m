...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if(indexPath.section == 0) {
		...
		if(indexPath.row == 0) {
			...
			cell.textLabel.text = NSLocalizedString(@"About eMAILsearch", nil);
	...
	if(indexPath.section == 1) {
		...
		if(indexPath.row == 0) {
			...
			UIAlertView* av = [[[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Clear Attachments?",nil)
														 message:NSLocalizedString(@"This will delete all attachments downloaded to eMAILsearch.", nil) 
														delegate:d cancelButtonTitle:@"No" otherButtonTitles:@"Yes", nil] autorelease];
			[av show];
		}		
		...
		if(indexPath.row == 0) {
			if ([MFMailComposeViewController canSendMail]) {
				...
				[mailCtrl setToRecipients:[NSArray arrayWithObject:@"support@ninetydegreesnorth.com"]];
				[mailCtrl setMessageBody:body isHTML:NO];
				[mailCtrl setSubject:@"eMAILsearch Feedback"];
				
				[self presentModalViewController:mailCtrl animated:YES];
				[mailCtrl release];
			} else {
				WebViewController* vc = [[WebViewController alloc] init];
				vc.title = NSLocalizedString(@"Love eMAILsearch?",nil);
				vc.serverUrl = @"http://itunes.apple.com/WebObjects/MZStore.woa/wa/viewContentsUserReviews?id=360593251&onlyLatestVersion=true&pageNumber=0&sortOrdering=1&type=Purple+Software";
			...
			vc.title = NSLocalizedString(@"About eMAILsearch", nil);
			vc.toolbarItems = [self.toolbarItems subarrayWithRange:NSMakeRange(0, 2)];
			[self.navigationController pushViewController:vc animated:YES];
			[vc release];
		}
