		...
		for (int i = 0; i < [messages count]; i++) {
			...
			@try  {
				...
				if(![sm serverReachable:self.accountNum]) {
					...
					NSString* detail = [NSString stringWithFormat:NSLocalizedString(@"Email that caused error:\nSubject: %@\nSender: %@\nDate: %@\nUDID: %@\nSeq/Start/End/Total/New/E:%i/%i/%i/%i/%i/%@.\n\nWays to fix this:\n1. Try syncing again :-)\n2. Hit Skip button below\n3. Delete this email or move it to a location that isn't downloaded by eMAILsearch.\n4. Copy and email error to support@ninetydegreesnorth.com", nil), 
