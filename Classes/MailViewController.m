...
-(NSString*)replyString {
	...
	if([AppSettings promo]) {
		NSString* promoLine = NSLocalizedString(@"I found your email with eMAILsearch: http://ninetydegreesnorth.com/emailsearch.html", nil);
...
-(int)peopleList:(NSString*)title addToView:(UIView*)addToView peopleList:(NSArray*)peopleList top:(int)top highlightAll:(BOOL)highlightAll highlightQuery:(NSString*)highlightQuery {
	...
	toView.frame = CGRectMake(0,top,self.scrollView.size.width,100);
	toView.autoresizingMask = UIViewAutoresizingFlexibleWidth;
	...
	line.frame = CGRectMake(0,toView.bottom,self.scrollView.size.width,1);
...
-(int)subjectBlock:(NSString*)subject markedUp:(NSString*)subjectMarkedUp date:(NSDate*)date top:(int)top addToView:(UIView*)addToView showTTView:(BOOL)showTTView {
	...
	if(showTTView) {
		self.subjectTTLabel = [[[TTStyledTextLabel alloc] initWithFrame:CGRectMake(5, top, self.scrollView.size.width-65-5, 21)] autorelease];
		...
		self.subjectTTLabel.autoresizingMask = UIViewAutoresizingFlexibleWidth;
		[addToView addSubview:self.subjectTTLabel];
	}
	
	CGRect subjectUIRect = CGRectMake(-3, top, self.scrollView.size.width-10, 21);
	if(showTTView) {
		subjectUIRect = CGRectMake(-3, top, self.scrollView.size.width-62-10, 21);
	}
	...
	UIImageView* line = [[[UIImageView alloc] init] autorelease];
	line.backgroundColor = [UIColor darkGrayColor];
	line.frame = CGRectMake(0,labelDate.bottom+1,self.scrollView.size.width,1);
	line.autoresizingMask = UIViewAutoresizingFlexibleWidth;
	[addToView addSubview:line];

	if(showTTView) {
		self.copyModeLabel = [[[UILabel alloc] initWithFrame:CGRectMake(self.subjectTTLabel.right-1, top+22, 61, 17)] autorelease];
		self.copyModeLabel.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin;
		...
		self.copyModeButton.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin;
...
-(void)loadedEmail {
	...
	if(self.email == nil) {
		...
		self.scrollView.contentSize = CGSizeMake(self.scrollView.contentSize.width, bottom+2);
...
- (void)loadView {
	self.view = [[[[UIView class] alloc] initWithFrame:CGRectMake(0,0,320,416)] autorelease];
	self.view.backgroundColor = [UIColor blackColor];
		
	// This is our real view, dude
	self.scrollView = [[[[UIScrollView class] alloc] initWithFrame:CGRectMake(0,0,320,416)] autorelease];
	self.scrollView.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
	...
	self.scrollView.contentSize = CGSizeMake(self.scrollView.contentSize.width, 180);
