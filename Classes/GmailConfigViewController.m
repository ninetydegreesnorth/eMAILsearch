...
-(IBAction)loginClick {
	...
	if(self.newAccount) {
		...
		if([self accountExists:username server:server]) {
			UIAlertView* alertView = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Account Exists",nil) message:NSLocalizedString(@"eMAILsearch already has an account for this username/server combination", nil) delegate:self cancelButtonTitle:@"OK" otherButtonTitles: nil];
			[alertView show];
			[alertView release];
			return;
		}
