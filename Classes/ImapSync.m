...
-(void)run {	
	...
	if(username == nil || [username length] == 0 || password == nil) {
		[sm syncAborted:[NSString stringWithFormat:NSLocalizedString(@"Incomplete credentials for account %i", nil), self.accountNum] detail:@"One of your accounts has an empty username or password."];
		[pool release];
		return;
	}
